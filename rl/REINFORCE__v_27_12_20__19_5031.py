...
class REINFORCE:
    ...
    def build_policy_network(self):
        ...
        model_shape_queue_0 = Conv2D(
            3, (3, 3), activation="relu")(input_shape_queue_0)
        model_shape_queue_0 = Conv2D(
            3, (3, 3), activation="relu")(model_shape_queue_0)
        ...
        model_shape_queue_1 = Conv2D(
            3, (3, 3), activation="relu")(input_shape_queue_1)
        model_shape_queue_1 = Conv2D(
            3, (3, 3), activation="relu")(model_shape_queue_1)
        model_shape_queue_1 = Flatten()(model_shape_queue_1)
        model_shape_queue_1 = Model(
            inputs=input_shape_queue_1, outputs=model_shape_queue_1, name="Model_ShapeQueue_1")

        model_shape_queue_2 = Conv2D(
            3, (3, 3), activation="relu")(input_shape_queue_2)
        model_shape_queue_2 = Conv2D(
            3, (3, 3), activation="relu")(model_shape_queue_2)
        model_shape_queue_2 = Flatten()(model_shape_queue_2)
        model_shape_queue_2 = Model(
            inputs=input_shape_queue_2, outputs=model_shape_queue_2, name="Model_ShapeQueue_2")

        combined = concatenate([model_game_grid.output, model_shape_queue_0.output,
                                model_shape_queue_1.output, model_shape_queue_2.output])
        ...
        model_output = Model(inputs=[model_game_grid.input, model_shape_queue_0.input,
                                     model_shape_queue_1.input, model_shape_queue_2.input], outputs=model_output, name="Model_Output")
    ...
    def compute_action(self, uid):
        ...
        img_game_grid = cv2.imread(
            f"{self.env.game_view.base_dictionary}/game_gird/game_gird_{uid}.jpg")
        img_shape_queue_0 = cv2.imread(
            f"{self.env.game_view.base_dictionary}/queue_shapes/queue_shapes_{uid}0.jpg")
        img_shape_queue_1 = cv2.imread(
            f"{self.env.game_view.base_dictionary}/queue_shapes/queue_shapes_{uid}1.jpg")
        img_shape_queue_2 = cv2.imread(
            f"{self.env.game_view.base_dictionary}/queue_shapes/queue_shapes_{uid}2.jpg")
        ...
        input: list = [imgs_game_grid, imgs_shape_queue_0,
                       imgs_shape_queue_1, imgs_shape_queue_2]
    ...
    def train_policy_network(self):
        ...
        for iteration in self.uids:
            # Get the img
            img_game_grid = cv2.imread(
                f"{self.env.game_view.base_dictionary}/game_gird/game_gird_{iteration}.jpg")
            img_shape_queue_0 = cv2.imread(
                f"{self.env.game_view.base_dictionary}/queue_shapes/queue_shapes_{iteration}0.jpg")
            img_shape_queue_1 = cv2.imread(
                f"{self.env.game_view.base_dictionary}/queue_shapes/queue_shapes_{iteration}1.jpg")
            img_shape_queue_2 = cv2.imread(
                f"{self.env.game_view.base_dictionary}/queue_shapes/queue_shapes_{iteration}2.jpg")
        ...
        history = self.model.train_on_batch(
            [imgs_game_grid, imgs_shape_queue0, imgs_shape_queue1, imgs_shape_queue2], y_train)
...
# Conclusie
""" - In het begin doet hij willekeurige acties op verschillende plaatsen op de env maar na verloop van tijd neemt hij altijd actie 0 + hij zet deze actie altijd op dezelfde plek"""