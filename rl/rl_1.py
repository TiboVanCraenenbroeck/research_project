...
class REINFORCE:
    def __init__(self, path=None, replay_memory_size: int = 1000, port: int = 8080):
        ...
        self.tensorboard = TensorBoard(log_dir='./logs/{}'.format(self.agent_name))

        self.space_availible_shapes_true = collections.deque(maxlen=replay_memory_size)
        self.space_availible_shapes_false = collections.deque(maxlen=replay_memory_size)
        ...
        if not path:
            self.model_chance_space_availible = self.build_nn_chance_space_availible(-1, True)
    ...
    def save_model(self):
        ...
        self.model_chance_space_availible.save(f"{BASE_DIR}/start_models/REINFORCE/{self.model_chance_space_availible_name}")
    ...
    def build_policy_network(self):
        ...
        models_output = []
        for i in range(100):
            model_single, input_shape_state, input_shape_shape = self.build_nn_chance_space_availible(i)
        ...
        """ model_output = Reshape((10, 10, 1))(model_output)
        
        model_output = Conv2D(2, (2, 2), activation="relu", name=f"conv2d_output")(model_output)
        ...
        model_output = Dense(100, activation="softmax", name="dense_output")(model_output)
    ...
    def remember(self,state, shape ,uid, action, action_prob, reward, action_shape):
        ...
        action_r = 0 if action_shape==0 else 100 if action_shape==1 else 200
        action_prob = action_prob[action_r:action_r + 100]
    ...
    def compute_action(self, states, shape_queue_0, shape_queue_1, shape_queue_2):
        ...
        for shape in shape_queue:
            input_nn = [np.array([output]) for state in states for output in (state, shape)]

            action_probability_distribution_shape = self.model.predict(input_nn).flatten()
            action_probability_distribution = np.concatenate((action_probability_distribution,action_probability_distribution_shape), axis=None)

        # Norm action probability distribution
        action_probability_distribution /= np.sum(action_probability_distribution)
        
        # Sample action
        action = np.random.choice(self.action_shape_total, 1, p=action_probability_distribution)[0]
    ...
    def train_policy_network(self, nr_iterations_update_weights_left):
        ...
        if self.policy_nn_can_train:
            X_train = [np.array(output, dtype=np.float) for i in range(100) for output in (np.asarray(list(zip(*self.states))[i], dtype=float), self.shapes)]
    ...
    def train_chance_space_available_nn(self, nr_iterations_update_weights_left):
        ...
        if len(self.space_availible_shapes_true)>=self.batch_size and len(self.space_availible_shapes_false)>=self.batch_size and nr_iterations_update_weights_left%self.train_chance_space_available_nn_after==0:
            samples_true = random.sample(self.space_availible_shapes_true, self.batch_size)
            samples_false = random.sample(self.space_availible_shapes_false, self.batch_size)
            mini_batches = samples_true + samples_false
            X_train_state = np.asarray(list(zip(*mini_batches))[0], dtype=float)
            X_train_shape = np.asarray(list(zip(*mini_batches))[1], dtype=float)
            y_train = np.asarray(list(zip(*mini_batches))[2], dtype=float)

            self.model_chance_space_availible.fit([X_train_state, X_train_shape], y_train, epochs=1, verbose=0)
    ...
    def update_chance_space_availible_policy_nn(self):
        ...
        if len(self.space_availible_shapes_true)>=self.batch_size and len(self.space_availible_shapes_false)>=self.batch_size:
            ...
            y_pred = self.model_chance_space_availible.predict([X_test_state, X_test_shape])
            ...
            if self.policy_nn_can_update:
                ...
                self.batch_size = 128
                updated_weights = ["input_state_", "input_shape_", "add_", "dropout_", "conv2d_", "flatten_", "dense_"]
                layer_dict = {layer.name:layer for layer in self.model_chance_space_availible.layers}
    ...
    def convert_state_to_5_5(self, state):
        ...
        for i_row, row in enumerate(state):
            for i_col, col in enumerate(row):
                ...
                standard_state[0:state_5_5.shape[0], 0:state_5_5.shape[1]] = state_5_5
    ...
    def train(self, episodes):
        ...
        for episode in range(episodes):
            ...
            shapes_queue_0, shapes_queue_1, shapes_queue_2 = self.change_state_shape_queue(shapes_queue)
            ...
            while not done:
                action, prob = self.compute_action(states_5_5, shapes_queue_0, shapes_queue_1, shapes_queue_2)
                ...
                shapes_queue_0_new, shapes_queue_1_new, shapes_queue_2_new = self.change_state_shape_queue(shapes_queue_new)
                ...
                action = int(f"{action_row}{action_col}")
                # Add to memory for the policynetwork
                self.remember(states_5_5, shapes_queue[action_shape], uid, action, prob, reward, action_shape)
                # Add to memory for the shape-nn
                sa = True if reward > 0 else False
                sa_nr = 1 if reward > 0 else 0
                self.add_to_memory(sa, states_5_5[action], shapes_queue[action_shape], sa_nr)
                self.train_chance_space_available_nn(nr_iterations_update_weights_left)
                ...
                if sa:
                    ...
                    nr_actions_false += 1
                self.env.render()
            history = self.train_policy_network(nr_iterations_update_weights_left)
