...
class DQNAgent:
    ...
    def build_nn(self):
        ...
        model_output = Conv2D(5, (1, 1), kernel_initializer=initializers.he_normal())(model_output)
        ...
        model_output = Dense(300,kernel_initializer=initializers.he_normal())(model_output)
        model_output = LeakyReLU()(model_output)
        

        model_output = Dense(len(self.possible_actions), activation="linear")(model_output)
        ...
        model_output.compile(optimizer=Adam(lr=0.001), loss="mse", metrics=["MeanSquaredError"])
        return model_output
    ...
    def policy_nn_train(self):
        ...
        if len(self.replay_memory)>= self.batch_size:
            ...
            mini_batches_states = np.asarray(list(zip(*mini_batches))[0], dtype=float)
            mini_batches_shapes_0 = np.asarray(list(zip(*mini_batches))[1], dtype=float)
            mini_batches_shapes_1 = np.asarray(list(zip(*mini_batches))[2], dtype=float)
            mini_batches_shapes_2 = np.asarray(list(zip(*mini_batches))[3], dtype=float)
            mini_batches_next_states = np.asarray(list(zip(*mini_batches))[6], dtype=float)
            mini_batches_next_shapes_0 = np.asarray(list(zip(*mini_batches))[7], dtype=float)
            mini_batches_next_shapes_1 = np.asarray(list(zip(*mini_batches))[8], dtype=float)
            mini_batches_next_shapes_2 = np.asarray(list(zip(*mini_batches))[9], dtype=float)
            mini_batches_actions = np.asarray(list(zip(*mini_batches))[4], dtype=int)
            mini_batches_rewards = np.asarray(list(zip(*mini_batches))[5], dtype=float)
            mini_batches_done = np.asarray(list(zip(*mini_batches))[10], dtype=bool)

            q_values_current_state = self.policy_nn_predict(mini_batches_states, mini_batches_shapes_0, mini_batches_shapes_1, mini_batches_shapes_2)
            y_train = q_values_current_state

            q_values_next_state = self.target_nn_predict(mini_batches_next_states, mini_batches_next_shapes_0, mini_batches_next_shapes_1, mini_batches_next_shapes_2)
            ...
            for i, done in enumerate(mini_batches_done):
                if done:
                    ...
                    y_train[i, mini_batches_actions[i]] = mini_batches_rewards[i] + self.discount * q_value_max_next_state[i]

            self.policy_nn_model.fit([mini_batches_states, mini_batches_shapes_0, mini_batches_shapes_1, mini_batches_shapes_2], y_train, batch_size=self.batch_size, verbose=0, use_multiprocessing=True, epochs=1)
    ...
    def policy_nn_predict(self, state, shape_0, shape_1, shape_2):
        ...
        self.q_policy = self.policy_nn_model.predict([state, shape_0, shape_1, shape_2])
    ...
    def target_nn_predict(self, state, shape_0, shape_1, shape_2):
        ...
        self.shape_1 = shape_1
        self.shape_2 = shape_2
        self.q_target = self.target_nn_model.predict([state, shape_0,shape_1, shape_2])
    ...
    def add_step_to_replay_memory(self, state, shape_0, shape_1, shape_2, action, reward, new_state, new_shape_0, new_shape_1, new_shape_2, done):
        step = (state, shape_0, shape_1, shape_2, action, reward, new_state, new_shape_0, new_shape_1, new_shape_2, done)
    ...
    def change_exploration_rate(self):
        if self.exploration == True:
            self.exploration_rate = self.exploration_number - (self.episode_nr * self.decay)
            self.episode_nr += 1
    ...
    def compute_action(self, state, shapes_queue_0, shapes_queue_1, shapes_queue_2):
        ...
        if np.random.uniform()<= self.exploration_rate:
            ...
            q_values = self.policy_nn_predict(state, shapes_queue_0, shapes_queue_1, shapes_queue_2)
    ...
    def save_agent(self, agent):
        ...
        agent_t.policy_nn_model.save(f'{self.base_path}/policy_model_agent_{self.agent_name}.h5')
        agent_t.target_nn_model.save(f'{self.base_path}/target_model_agent_{self.agent_name}.h5')
    ...
    def get_agent(self, agent):
      ...
      agent.policy_nn_model.load_weights(f'{self.base_path}/policy_model_agent_{self.agent_name}.h5')
      agent.target_nn_model.load_weights(f'{self.base_path}/target_model_agent_{self.agent_name}.h5')
      return agent
...
agent = DQNAgent(replay_memory_size, batch_size, possible_actions, nr_episodes_update_target_nn, epsilon, min_epsilon, discount, decay)
...
for i in range(agent.nr_episodes):
    ...
    while not done:
        total_steps +=1
        action = agent.compute_action(state, shapes_queue_0, shapes_queue_1, shapes_queue_2)
        action_shape, action_row, action_col = number_to_arc(action)
        total_actions.append(action)
        reward, full_lines, new_state, done, new_shape, uid = env.step(env.shapes_queue[action_shape], action_row, action_col)

        new_shape = [create_standard_shape(shape) for shape in new_shape]
        new_state, new_shape_0, new_shape_1, new_shape_2 = change_state_shape_queue(new_state, new_shape)
        ...
        agent.add_step_to_replay_memory(state,shapes_queue_0, shapes_queue_1, shapes_queue_2, action, reward, new_state, new_shape_0, new_shape_1, new_shape_2, done)
        agent.train()
