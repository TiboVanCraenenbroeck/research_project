...
class A2C:
    ...
    def build_nn(self):
        ...
        model_game_grid = Model(
            inputs=[input_game_grid], outputs=[model_game_grid])
        ...
        actor_learn = Model(
            inputs=[input_game_grid, input_shape, input_delta], outputs=[actor])
        ...
        actor_learn.compile(loss=custom_actor_loss,
                            optimizer=Adam(lr=self.learning_rate_actor))
        critic.compile(loss="mean_squared_error",
                       optimizer=Adam(lr=self.learning_rate_critic))
    ...
    def train_nn(self):
        if len(self.states) > 0:
            ...
            targets = [self.rewards[i] + self.gamma * next_values[i]
                       * (1-int(done)) for i, done in enumerate(self.done)]
            # TODO: Zet de ongeldige acties op 0!!!
            deltas = [target - value[i] for i, target in enumerate(targets)]
            ...
            self.actor_learn_nn.fit(
                [states, shapes, deltas], actions, epochs=1, verbose=0)
